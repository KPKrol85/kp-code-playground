...
export function validateMachineReadableReport(value) {
  ...
  Object.entries(value.scores).forEach(([key, item]) => { if (key === 'weightedScore') { if (item !== null) assertPercent(item); } else if (key === 'scoreAvailable') { if (typeof item !== 'boolean') throw new Error('Invalid score availability.'); } else assertCount(item); });
...
function assertObject(value, allowed) { if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).some((key) => !allowed.includes(key))) throw new Error('Invalid or unknown machine-readable report field.'); } function assertReference(value) { assertObject(value, ['id', 'name']); assertText(value.id); assertText(value.name); } function assertText(value) { if (typeof value !== 'string') throw new Error('Invalid report text.'); } function assertCount(value) { if (!Number.isInteger(value) || value < 0) throw new Error('Invalid report count.'); } function assertPercent(value) { if (!Number.isFinite(value) || value < 0 || value > 100) throw new Error('Invalid report percentage.'); } function assertArray(value, uniqueKey, validator) { if (!Array.isArray(value)) throw new Error('Invalid report array.'); const seen = new Set(); value.forEach((item) => { validator(item); if (!item[uniqueKey] || seen.has(item[uniqueKey])) throw new Error('Duplicate or missing report identifier.'); seen.add(item[uniqueKey]); }); }
