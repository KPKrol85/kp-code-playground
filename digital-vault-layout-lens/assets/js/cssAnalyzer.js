...
export const CSS_ANALYZER_SOURCE = 'css-analyzer';
...
export function parseCssDeclarations(css) {
  ...
  while ((match = re.exec(withoutComments))) {
    ...
    const declarations = body.split(';').map((part) => part.trim()).filter(Boolean).map((part) => {
      ...
      return { selector, property, value, normalizedValue: normalizeValue(value), inResponsive: isResponsiveContext(withoutComments, match.index) || RESPONSIVE_RE.test(value) };
    }).filter(Boolean);
...
function checkRepeatedLiterals(parsed) {
  ...
  return [...groups.entries()].filter(([, items]) => items.length >= CSS_ANALYZER_THRESHOLDS.repeatedLiteralMinimum).map(([value, items]) => finding('repeated-literal-values', 'Repeated literal CSS value', 'Visual consistency', 'low', `The literal value "${value}" appears ${items.length} times across CSS declarations. This may be a design-token opportunity, not an automatic error.`, { repeatedValue: value, occurrenceCount: items.length, properties: stableUnique(items.map((i) => i.property)) }));
}
...
function checkOverflowRisks(parsed) {
  ...
  if (overflowHidden.length) risks.push(finding('overflow-x-hidden', 'Horizontal overflow hidden', 'Responsive behavior', 'medium', 'overflow-x: hidden can mask content that does not reflow. Verify narrow viewports manually; this static check is a potential WCAG reflow risk only.', { occurrenceCount: overflowHidden.length }, [WCAG.reflow]));
  if (nowrap.length && !parsed.declarations.some((d) => d.property.startsWith('text-overflow') || d.property === 'overflow-wrap')) risks.push(finding('nowrap-without-overflow-strategy', 'Nowrap text without overflow strategy', 'Responsive behavior', 'medium', 'white-space: nowrap appears without an obvious text-overflow or wrapping fallback, which can create horizontal overflow for text.', { occurrenceCount: nowrap.length }, [WCAG.reflow]));
  if (rigidGrid.length) risks.push(finding('rigid-grid-tracks', 'Rigid wide grid tracks', 'Responsive behavior', 'medium', 'Grid columns use wide rigid pixel tracks that may exceed narrow viewports unless adapted elsewhere.', { occurrenceCount: rigidGrid.length }, [WCAG.reflow]));
  if (absFixed.length) risks.push(finding('positioned-fixed-width', 'Positioned element with large fixed width', 'Responsive behavior', 'medium', 'Fixed or absolute positioned elements with large fixed widths can escape narrow layouts.', { occurrenceCount: absFixed.length }, [WCAG.reflow]));
  return risks;
}

function finding(checkId, title, category, severity, message, metadata = {}, wcag) { return { issueId: createIssueId({ source: CSS_ANALYZER_SOURCE, ruleId: checkId, discriminator: stableDiscriminator(metadata) }), ruleId: checkId, checkId, title, category, severity, message, source: CSS_ANALYZER_SOURCE, findingType: 'static-css', occurrenceCount: metadata.occurrenceCount || 0, metadata, ...(wcag ? { wcag: cloneWcag(wcag) } : {}) }; }
