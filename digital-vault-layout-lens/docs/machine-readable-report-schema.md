...
Empty optional context and reviewer notes are omitted. Strings are NUL-free, whitespace-normalized plain text (notes retain bounded line breaks). Counts are non-negative integers; percentages are null or 0–100. Identifiers are unique in each relevant array. The serializer creates fresh frozen data and never serializes mutable application state.
