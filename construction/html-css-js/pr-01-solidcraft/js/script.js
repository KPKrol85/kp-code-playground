...
const runInit = () => {
  ...
  if (has('a[href="#top"], .scroll-top, [data-scroll="top"]')) initSmoothTop?.();
  ...
  if (has("#oferta .card picture img, .gallery .gallery-item picture img")) initOfertaLightbox?.();
  if (has('.services-track h3 a[href^="oferta/"], #oferta .card h3 a[href^="oferta/"]')) initOfferPrefetch?.();
  if (has("#kontakt") || has("#oferta") || has("#strona-glowna")) initHomeHelpers?.();
