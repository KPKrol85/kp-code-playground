...
async function run(mode) {
  ...
  for (const config of CONFIG) {
    ...
    if (!(await pathExists(inputDir))) {
      logger.debug(`images:${mode} skip missing input directory ${config.inputDir}`);
      continue;
    }
    ...
    for (const filePath of files) {
      ...
      if (mode === "build") {
        ...
        if (outputs.length > 0) {
          logger.debug(`images:build created ${outputs.length} output(s) from ${path.relative(PROJECT_ROOT, filePath)}`);
        }
        summary.push(...outputs);
      } else if (mode === "clean") {
        const removed = await cleanForFile(filePath, config);
        if (removed.length > 0) {
          logger.debug(`images:clean removed ${removed.length} output(s) from ${path.relative(PROJECT_ROOT, filePath)}`);
        }
  ...
  if (mode === "build") {
    logger.summary(`OK: images:build created ${summary.length} file(s) from ${scanned} scanned source file(s).`);
  } else if (mode === "clean") {
    logger.summary(`OK: images:clean removed ${summary.length} file(s) from ${scanned} scanned source file(s).`);
  }
}
