...
const getFocusableElements = (scope) =>
  qsa(FOCUSABLE_SELECTOR, scope).filter((element) => !element.hasAttribute("inert"));
...
export const initLegalModal = () => {
  ...
  const setModalVisibility = (isVisible) => {
    ...
    modal.toggleAttribute("inert", !isVisible);
  ...
  const alreadyAccepted = Boolean(localStorage.getItem(STORAGE_KEY));
  if (!alreadyAccepted) {
