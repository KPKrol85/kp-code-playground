...
| B2B ZUS presets | `full`, `preferential`, and `starter` presets are flat monthly social values plus health bases. | Simplified model assumption | 2026-07-01 | Presets may not reflect exact statutory amounts, eligibility, limits, or annual changes until officially verified. |
...
| B2B health contribution | Health contribution is computed as `healthBase * healthRate` for presets. | Simplified model assumption | 2026-07-01 | Does not model all health-contribution bases, minimums, annual settlement, revenue bands, or deductibility nuances. |
| B2B expenses | Business expenses are not deducted from taxable base and no expense input is active. | Simplified model assumption | 2026-07-01 | Planned future input only; do not treat current B2B taxable base as expense-aware. |
| VAT | `vatPayer` exists as an informational B2B option, but VAT cash flow is not included in calculation formulas or take-home pay. | Simplified model assumption | 2026-07-01 | UI and README state that VAT status does not change the current net result. |
| Lump-sum default/rate | Current lump-sum calculation uses `b2b.lumpSumRates.default` for all lump-sum cases. | Simplified model assumption | 2026-07-01 | Does not classify PKD/services, revenue bands, or individual eligibility; future selector is only planned in config boundaries. |
...
| B2B accounting costs | Accounting costs are not deducted and no accounting-cost input is active. | Simplified model assumption | 2026-07-01 | Future config boundaries mark accounting cost as planned only. |
